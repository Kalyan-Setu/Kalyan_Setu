...
export default function AdminSidebar() {
  ...
  return (
    <>
      {/* Mobile Top Horizontal Admin Bar (Sticky below main navbar on mobile) */}
      <div className="md:hidden w-full bg-surface-container-low border-b border-outline-variant px-2.5 py-2 sticky top-[73px] z-30 shadow-xs">
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`material-symbols-outlined text-[16px] ${isActive ? 'filled-icon' : ''}`}>
                  {item.icon}
                </span>
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 bg-surface-container-low border-r border-outline-variant flex-col shrink-0 min-h-[calc(100vh-5rem)] sticky top-20">
