      ...
      <main className="flex-1 p-3 sm:p-6 md:p-xl overflow-y-auto max-w-7xl w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md mb-md sm:mb-lg border-b border-outline-variant pb-md">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto bg-primary-container text-on-primary font-bold text-xs px-4 py-2.5 rounded hover:bg-primary transition-all flex items-center justify-center gap-1.5 min-h-[40px] cursor-pointer"
            >
              <span className="material-symbols-outlined text-sm">list_alt</span>
              <span>{t('admin.viewAllComplaints')}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-md mb-md sm:mb-lg">
          <div className="bg-surface-container-lowest border border-outline-variant p-3 sm:p-md rounded-lg shadow-ambient">
            <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold text-on-surface-variant mb-1">
              <span>{t('admin.kpi.totalGrievances')}</span>
            ...
            <div className="text-[10px] sm:text-[11px] text-gov-green font-semibold mt-1 flex items-center gap-1">
              ...
              {t('admin.kpi.dynamicQuery')}
            </div>
          </div>

          <div className="bg-surface-container-lowest border border-error-container p-3 sm:p-md rounded-lg shadow-ambient border-l-4 border-l-error">
            <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold text-error mb-1">
              <span>{t('admin.kpi.criticalAlerts')}</span>
              <span className="material-symbols-outlined text-error text-lg sm:text-xl">warning</span>
            </div>
            <div className="text-2xl sm:text-3xl font-bold text-error">{criticalCount}</div>
            <div className="text-[10px] sm:text-[11px] text-error font-semibold mt-1">{t('admin.kpi.immediateDispatch')}</div>
          </div>

          <div className="bg-surface-container-lowest border border-outline-variant p-3 sm:p-md rounded-lg shadow-ambient">
            <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold text-on-surface-variant mb-1">
              <span>{t('admin.kpi.activeProgress')}</span>
            ...
            <div className="text-[10px] sm:text-[11px] text-on-surface-variant mt-1">{t('admin.kpi.teamsDeployed')}</div>
          </div>

          <div className="bg-surface-container-lowest border border-outline-variant p-3 sm:p-md rounded-lg shadow-ambient">
            <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold text-on-surface-variant mb-1">
              <span>{t('admin.kpi.slaRate')}</span>
            ...
            <div className="text-[10px] sm:text-[11px] text-on-surface-variant mt-1">{t('admin.kpi.slaTarget')}</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-5 bg-surface-container-lowest border border-outline-variant rounded-lg p-4 sm:p-lg shadow-ambient flex flex-col">
            <div className="flex justify-between items-center mb-md border-b border-outline-variant pb-2">
              ...
              <button
                ...
                className="text-[11px] text-primary font-bold hover:underline py-1 px-2 cursor-pointer"
              >
                {t('navbar.takeAction')}
              </button>
            </div>
              ...
              {complaints.filter(c => c.status !== 'Deleted' && c.status !== 'Rejected').slice(0, 3).map((item) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="font-mono text-[11px] font-bold text-primary">#{item.display_id || item.id}</span>
                  ...
                  <div className="text-[10px] text-on-surface-variant flex items-center justify-between mt-1">
                    ...
                    <span className="text-primary font-semibold shrink-0">{t('navbar.takeAction')} →</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-4 sm:p-lg shadow-ambient">
          <h2 className="text-sm font-bold text-primary mb-md border-b border-outline-variant pb-2 flex items-center gap-1.5">
            ...
            <span>{t('footer.adminPortals')}</span>
          </h2>
            ...
            {departmentMetrics.map((dept) => (
              <div key={dept.name} className="bg-surface p-3 sm:p-md rounded-lg border border-outline-variant flex flex-col gap-2">
                ...
                <div className="grid grid-cols-3 gap-1 text-center bg-white p-2 rounded border border-outline-variant/60 text-[11px]">
                  <div>
                    <span className="text-[10px] text-on-surface-variant block">{t('common.all')}</span>
                  ...
                  <div>
                    <span className="text-[10px] text-on-surface-variant block">{t('common.details')}</span>
